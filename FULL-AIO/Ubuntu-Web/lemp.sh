...
if [ $USER != "root" ]; then
	...
	# Install PKG
	apt install -y curl openssl nginx certbot python3-certbot-nginx clamav clamav-daemon fail2ban net-tools ufw zip unzip mariadb-server php php-xml php-fpm php-cli php-curl php-mysql php-gd php-mbstring php-imagick php-intl php-xml php-zip php-cgi php-xmlrpc php-soap tidy php-tidy sqlite php-pear -y
	########		SERVER WEB		########
